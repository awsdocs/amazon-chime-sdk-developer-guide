...
Your SIP application always invokes its Lambda function after running this action, with an `ACTION_SUCCESSFUL` or `ACTION_FAILED` event type\. When the application successfully gathers digits, it sets the `ReceivedDigits` variable in the `ActionData` object\. The following code example shows the event structure of that Lambda function\.
